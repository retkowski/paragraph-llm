...
for partition in partitions:
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc=f"Processing {partition} partition"):
        ...
        for section_idx, section_sentences in enumerate(sections):
            ...
            section_paragraph_targets = ['0'] * len(section_sentences)
            ...
            for i in range(len(section_sentences) - 1):
                ...
                input_ids = get_input_ids(section_text, current_output)
                ...
                paragraph_break = top_token.item() in newline_ids
                if paragraph_break:
                    section_paragraph_targets[i] = '1'
            ...
            section_paragraph_targets[-1] = '1'

            assert len(section_paragraph_targets) == len(section_sentences), (f"Flag/ sentence length mismatch: {len(section_paragraph_targets)} vs {len(section_sentences)}")


            section_paragraphs = []
            current_par = []
            for sent, flag in zip(section_sentences, section_paragraph_targets):
                current_par.append(sent)
                if flag == '1':
                    section_paragraphs.append(current_par)
                    current_par = []
            if current_par:
                section_paragraphs.append(current_par)
            ...
            paragraph_targets.extend(section_paragraph_targets)
